...
router.get(
    '/partner/:id',
    (req, res) => {
        storage.findPartner(req.params.id).pipe(
            switchMap(a => isNotDynamodbError(a) ? of(a) : throwError(a))
        ).subscribe(
            (partner) => {
                res.json(partner);
...
router.get(
    '/partner/:id/location',
    ((req, res) => {
        locationstorage.findLocations(
            req.params.id,
            100,
            null
        ).pipe(
            switchMap(a => isNotDynamodbError<Page<Location>>(a) ? of(a) : throwError(a))
        ).subscribe(
            locations => {
                res.json(locations);
            },
...
router.put(
    ...
    (req, res) => {
        ...
        storage.hidePartner(
            req.params.id,
            req.query.hide === 'true'
        ).pipe(switchMap(a => isNotDynamodbError(a) ? of(a) : throwError(a))).subscribe(
            elem => {
...
router.get(
    ...
    (req, res) => {
        ...
        monthlyReportStorage.findPaged(req.params.partnerId).pipe(
            switchMap(a => isNotDynamodbError(a) ? of(a) : throwError(a))
        ).subscribe(
            elem => {
                res.json(elem);
